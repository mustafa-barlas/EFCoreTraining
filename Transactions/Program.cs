...
Person p = new() { Name = "Cevat" };
await context.Persons.AddAsync(p);
await context.SaveChangesAsync(); 
await transaction.CommitAsync();  // veritabanına iletilmesi için commit edilmeli
...
await transaction1.CreateSavepointAsync("t1");

Person? p10 = await context.Persons.FindAsync(10);
context.Persons.Remove(p10);
await context.SaveChangesAsync();

await transaction1.RollbackToSavepointAsync("t1");
